# Changelog for CompassBuildingRome
...
This is a fairly massive overhaul of CompassBuildingRome. There are breaking changes, so please be aware if you're planning on upgrading an older CompassBuildingRome-based theme. The most noticeable change is the fact that we've gotten rid of the tiered directory structure in favor of a more standard flat directory structure. This was done primarily to make child theming and deploying easier.

Because so much has changed in this version. I'm not going to point everything out in the release, but the following is a list of all major changes since the last version. For a full breakdown, read the [commit history](https://github.com/FlagshipWP/compassbuildingrome/commits/master).
...
Not a whole lot has changed feature-wise in this release, but a number of improvements have been made under the hood. In addition to the master and develop branches, we're now also maintaining a [node-sass branch](https://github.com/FlagshipWP/compassbuildingrome/tree/node-sass) which compiles faster than the Ruby Sass version.
...
- The `compassbuildingrome_footer_creds` function has been deprecated and will be removed on the next update.
